...
namespace Dungeons_.Maps
{
    public class DamageBaff
    {
        ...
        Box box;
        public DamageBaff(int x, int y, Image sprite, Image boxSprite, bool box)
        {
            this.x = x;
            this.y = y;
            this.width = 36;
        ...
        public void Draw(Graphics g, Camera camera, Student student)
        {
            ...
            if (IsVisible)
            {
                ...
                {
                    g.DrawImage(Sprite, new Rectangle(new Point(x + camera.X, y + camera.Y), new Size(width, height)), width * currFrame + 72, 0, width, height, GraphicsUnit.Pixel);
